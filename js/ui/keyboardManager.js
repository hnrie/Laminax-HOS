...
const { Clutter, Gio, GLib, GObject, IBus, Meta, Laminax, St, LaminaxDesktop } = imports.gi;
...
DESKTOP_INPUT_SOURCES_SCHEMA = 'org.Laminax.desktop.input-sources';
...
function getXkbInfo() {
    if (_xkbInfo == null)
        _xkbInfo = new LaminaxDesktop.XkbInfo();
    return _xkbInfo;
}
...
var KeyboardManager = class {
    ...
    _getLocaleLayout() {
        ...
        let [found, , id] = LaminaxDesktop.get_input_source_from_locale(locale);
        if (!found)
            [, , id] = LaminaxDesktop.get_input_source_from_locale(DEFAULT_LOCALE);
...
var InputSourceManager = class {
    constructor() {
        ...
        this._kb_settings = new Gio.Settings({ schema_id: "org.Laminax.desktop.keybindings.wm" });
        this._interface_settings = new Gio.Settings({ schema_id: "org.Laminax.desktop.interface" });
    ...
    _currentInputSourceChanged(newSource) {
        ...
        Main.LaminaxDBusService.EmitCurrentInputSourceChanged(newSource.id);
    ...
    _inputSourcesChanged() {
        ...
        Main.LaminaxDBusService.EmitInputSourcesChanged();
    }
