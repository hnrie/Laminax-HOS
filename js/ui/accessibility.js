...
A11yHandler.prototype = {
    _init: function() {
        this.a11y_keyboard_settings = new Gio.Settings( { schema_id: "org.Laminax.desktop.a11y.keyboard" });
        this.a11y_mouse_settings = new Gio.Settings( { schema_id: "org.Laminax.desktop.a11y.mouse" });
        this.wm_settings = new Gio.Settings( { schema_id: "org.Laminax.desktop.wm.preferences" });
    ...
    popup_state_osd: function(key, state) {
        ...
        switch (key) {
            case CAPS:
                icon = Gio.ThemedIcon.new(state ? "Laminax-caps-lock-symbolic" :
                                                  "Laminax-caps-lock-off-symbolic");
                text = state ? _("Caps Lock On"):
                               _("Caps Lock Off");
                break;
            case NUM:
                icon = Gio.ThemedIcon.new(state ? "Laminax-num-lock-symbolic" :
                                                  "Laminax-num-lock-off-symbolic");
