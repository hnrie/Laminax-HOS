...
const LaminaxIface =
    '<node> \
        <interface name="org.Laminax"> \
            ...
            <property name="LaminaxVersion" type="s" access="read" /> \
...
function LaminaxDBus() {
    this._init();
}

LaminaxDBus.prototype = {
    _init: function() {
        this._dbusImpl = Gio.DBusExportedObject.wrapJSObject(LaminaxIface, this);
    ...
    ScreenshotArea: function(include_cursor, x, y, width, height, flash, filename) {
        let screenshot = new Laminax.Screenshot();
    ...
    updateSetting: function(uuid, instance_id, key, payload) {
        if (!Main.settingsManager.uuids[uuid]) {
            global.logWarning(
                `[LaminaxDBus] [${uuid}] Unable to find UUID from SettingsManager - ` +
    ...
    RestartLaminax: function(showOsd) {
        Main.restartLaminax(showOsd);
    },
    ...
    LaminaxVersion: Config.PACKAGE_VERSION
};
