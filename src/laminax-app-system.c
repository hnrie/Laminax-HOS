...
struct _LaminaxAppSystemPrivate {
  GMenuTree *apps_tree;
...
static void Laminax_app_system_finalize (GObject *object);
static void on_apps_tree_changed_cb (GMenuTree *tree, gpointer user_data);
LaminaxApp * lookup_heuristic_basename (LaminaxAppSystem *system, const char *name);
gchar *strip_extension (gchar *wm_class);
gboolean case_insensitive_search (const char *key,
                                  const char *value,
                                  gpointer user_data);

G_DEFINE_TYPE(LaminaxAppSystem, Laminax_app_system, G_TYPE_OBJECT);

static void Laminax_app_system_class_init(LaminaxAppSystemClass *klass)
{
  GObjectClass *gobject_class = (GObjectClass *)klass;

  gobject_class->finalize = Laminax_app_system_finalize;

  signals[APP_STATE_CHANGED] = g_signal_new ("app-state-changed",
                                             Laminax_TYPE_APP_SYSTEM,
                                             G_SIGNAL_RUN_LAST,
                                             0,
                                             NULL, NULL, NULL,
                                             G_TYPE_NONE, 1,
  ...
  signals[INSTALLED_CHANGED] =
    g_signal_new ("installed-changed",
		  Laminax_TYPE_APP_SYSTEM,
		  G_SIGNAL_RUN_LAST,
		  G_STRUCT_OFFSET (LaminaxAppSystemClass, installed_changed),
		  NULL, NULL, NULL,
		  G_TYPE_NONE, 0);

  g_type_class_add_private (gobject_class, sizeof (LaminaxAppSystemPrivate));
}
...
setup_merge_dir_symlink(void)
{
    ...
    sym_path = g_build_filename (user_config, "menus", "Laminax-applications-merged", NULL);
...
static void
Laminax_app_system_init (LaminaxAppSystem *self)
{
  LaminaxAppSystemPrivate *priv;

  self->priv = priv = G_TYPE_INSTANCE_GET_PRIVATE (self,
                                                   Laminax_TYPE_APP_SYSTEM,
                                                   LaminaxAppSystemPrivate);
  ...
  priv->flatpak_id_to_app = g_hash_table_new_full (g_str_hash, g_str_equal,
                                                         g_free,
                                                         (GDestroyNotify)g_object_unref);
/* According to desktop spec, since our menu file is called 'Laminax-applications', our
 * merged menu folders need to be called 'Laminax-applications-merged'.  We'll setup the folder
 ...
 */
   ...
   * case by case.
   */
  priv->apps_tree = gmenu_tree_new ("Laminax-applications.menu", GMENU_TREE_FLAGS_INCLUDE_NODISPLAY);
...
static void
Laminax_app_system_finalize (GObject *object)
{
  LaminaxAppSystem *self = Laminax_APP_SYSTEM (object);
  LaminaxAppSystemPrivate *priv = self->priv;
  ...
  G_OBJECT_CLASS (Laminax_app_system_parent_class)->finalize (object);
}
...
static gboolean
apps_have_same_name_and_exec (LaminaxApp *app1,
                              LaminaxApp *app2)
{
    const gchar *name1, *name2;
    const gchar *exec1, *exec2;

    name1 = Laminax_app_get_name (app1);
    name2 = Laminax_app_get_name (app2);
    ...
    exec1 = _Laminax_app_get_executable (app1);
    exec2 = _Laminax_app_get_executable (app2);

    return g_strcmp0 (exec1, exec2) == 0;
}

static void
rename_app (LaminaxApp *app,
            gpointer     user_data,
            gboolean     flatpak_iteration)
{
  ...
  if (_Laminax_app_get_unique_name (app) != NULL)
    {
      DEBUG_RENAMING ("      Skipping rename of app at %s - it is already modified (%s)\n",
                      Laminax_app_get_id (app),
                      _Laminax_app_get_unique_name (app));

      return;
    }

  if (Laminax_app_get_is_flatpak (app))
    {
      ...
      _Laminax_app_set_unique_name (app, unique_name);
    }
  ...
  basename = g_path_get_basename (_Laminax_app_get_executable (app));
  ...
  if (g_strcmp0 (capitalized_exec, common_name) == 0)
    {
      ...
      _Laminax_app_set_unique_name (app, g_strdup (common_name));
  ...
  for (i = 0; i < data->group->len; i++)
    {
      LaminaxApp *other_app = g_ptr_array_index (data->group, i);
      ...
      if (apps_have_same_name_and_exec (app, other_app))
        {
          DEBUG_RENAMING ("        Hiding app at '%s' as functional duplicate to app at '%s'\n",
                          Laminax_app_get_id (app),
                          Laminax_app_get_id (other_app));

          _Laminax_app_set_hidden_as_duplicate (other_app, TRUE);
        }
    }
}
...
still_has_names_duplicated (RenameAppData *data)
{
    ...
    for (i = 0; i < data->group->len; i++)
      {
        LaminaxApp *app = g_ptr_array_index (data->group, i);

        if (!g_hash_table_add (name_register, (gpointer) Laminax_app_get_name (app)))
          {
            duplicated = TRUE;
            break;
          }
...
on_apps_tree_changed_cb (GMenuTree *tree,
                         gpointer   user_data)
{
  LaminaxAppSystem *self = Laminax_APP_SYSTEM (user_data);
  GError *error = NULL;
  ...
  while (g_hash_table_iter_next (&iter, &key, &value))
    {
      ...
      if (app != NULL)
        {
           ...
           * the new data.
           */
          old_entry = Laminax_app_get_tree_entry (app);
          gmenu_tree_item_ref (old_entry);

#if DEBUG_APPSYS_RENAMING
          if (g_strcmp0 (_Laminax_app_get_desktop_path (app),
                         gmenu_desktopappinfo_get_filename (gmenu_tree_entry_get_app_info (entry))) == 0)
            {
              DEBUG_RENAMING ("Existing match found for app: '%s'.  Source unchanged ('%s')\n",
                              _Laminax_app_get_common_name (app),
                              _Laminax_app_get_desktop_path (app));
          ...
          _Laminax_app_set_entry (app, entry);
      ...
      if (Laminax_app_get_is_flatpak (app))
      {
        gchar *flatpak_app_id = Laminax_app_get_flatpak_app_id (app);
      ...
      info = Laminax_app_get_app_info (app);
      if (info)
        {
      ...
      if (!Laminax_app_get_nodisplay (app))
        {
          GPtrArray *same_name_apps;
          const char *common_name;

          common_name = _Laminax_app_get_common_name (app);

          DEBUG_RENAMING ("Adding '%s' (at %s) to check for uniqueness\n",
                          _Laminax_app_get_common_name (app), id);
...
/**
 * Laminax_app_system_get_tree:
 *
 * Return Value: (transfer none): The #GMenuTree for apps
 */
GMenuTree *
Laminax_app_system_get_tree (LaminaxAppSystem *self)
{
  return self->priv->apps_tree;
}

/**
 * Laminax_app_system_get_default:
 *
 * Return Value: (transfer none): The global #LaminaxAppSystem singleton
 */
LaminaxAppSystem *
Laminax_app_system_get_default (void)
{
  static LaminaxAppSystem *instance = NULL;

  if (instance == NULL)
    instance = g_object_new (Laminax_TYPE_APP_SYSTEM, NULL);
...
/**
 * Laminax_app_system_lookup_app:
 *
 * Find a #LaminaxApp corresponding to an id.
 *
 * Return value: (transfer none): The #LaminaxApp for id, or %NULL if none
 */
LaminaxApp *
Laminax_app_system_lookup_app (LaminaxAppSystem   *self,
                             const char       *id)
{
  LaminaxApp *result;
 ...
 */
LaminaxApp *
lookup_heuristic_basename (LaminaxAppSystem *system,
                                            const char     *name)
{
  LaminaxApp *result;
  GSList *prefix;

  result = Laminax_app_system_lookup_app (system, name);
  if (result != NULL)
    return result;
  ...
  for (prefix = system->priv->known_vendor_prefixes; prefix; prefix = g_slist_next (prefix))
    {
      ...
      result = Laminax_app_system_lookup_app (system, tmpid);
      g_free (tmpid);
      if (result != NULL)
        return result;
    }
...
/**
 * Laminax_app_system_lookup_desktop_wmclass:
 * @system: a #LaminaxAppSystem
 * @wmclass: (nullable): A WM_CLASS value
 *
 * Find a valid application whose .desktop file, without the extension
 * and properly canonicalized, matches @wmclass.
 *
 * Returns: (transfer none): A #LaminaxApp for @wmclass
 */
LaminaxApp *
Laminax_app_system_lookup_desktop_wmclass (LaminaxAppSystem *system,
                                            const char        *wmclass)
{
  char *canonicalized;
...
/**
 * Laminax_app_system_lookup_startup_wmclass:
 * @system: a #LaminaxAppSystem
 * @wmclass: (nullable): A WM_CLASS value
 *
 ...
 */
LaminaxApp *
Laminax_app_system_lookup_startup_wmclass (LaminaxAppSystem *system,
                                            const char     *wmclass)
{
  if (wmclass == NULL)
    return NULL;
...
/**
 * Laminax_app_system_lookup_flatpak_app_id:
 *
 * Find a #LaminaxApp corresponding to a flatpak app id.
 *
 * Return value: (transfer none): The #LaminaxApp for app_id, or %NULL if none
 */
LaminaxApp *
Laminax_app_system_lookup_flatpak_app_id (LaminaxAppSystem *system,
                                           const char        *app_id)
{
  if (app_id == NULL)
    return NULL;
...
/**
 * Laminax_app_system_get_all:
 * @system:
 *
 * Returns: (transfer container) (element-type LaminaxApp): All installed applications
 */
GSList *
Laminax_app_system_get_all (LaminaxAppSystem  *self)
{
  GSList *result = NULL;
  GHashTableIter iter;
  gpointer key, value;
  ...
  while (g_hash_table_iter_next (&iter, &key, &value))
    {
      LaminaxApp *app = value;

      if (!gmenu_desktopappinfo_get_nodisplay (Laminax_app_get_app_info (app)))
...
void
_Laminax_app_system_notify_app_state_changed (LaminaxAppSystem *self,
                                            LaminaxApp       *app)
{
  LaminaxAppState state = Laminax_app_get_state (app);
  ...
  switch (state)
    ...
    case Laminax_APP_STATE_STOPPED:
      break;
    default:
      g_warning("Laminax_app_system_notify_app_state_changed: default case");
    break;
    }
  g_signal_emit (self, signals[APP_STATE_CHANGED], 0, app);

  if (state == Laminax_APP_STATE_STOPPED)
    {
...
/**
 * Laminax_app_system_get_running:
 * @self: A #LaminaxAppSystem
 *
 * Returns the set of applications which currently have at least one
 * open window in the given context.
 *
 * Returns: (element-type LaminaxApp) (transfer container): Active applications
 */
GSList *
Laminax_app_system_get_running (LaminaxAppSystem *self)
{
  gpointer key, value;
  GSList *ret;
  GHashTableIter iter;
