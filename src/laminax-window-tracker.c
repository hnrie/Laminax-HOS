...
/**
 * SECTION:Laminax-window-tracker
...
struct _LaminaxWindowTracker
{
  GObject parent;

  LaminaxGlobal *global;

  LaminaxApp *focus_app;

  /* <MetaWindow * window, LaminaxApp *app> */
...
G_DEFINE_TYPE (LaminaxWindowTracker, Laminax_window_tracker, G_TYPE_OBJECT);
...
static void Laminax_window_tracker_finalize (GObject *object);
static void set_focus_app (LaminaxWindowTracker  *tracker,
                           LaminaxApp            *new_focus_app);
static void on_focus_window_changed (MetaDisplay *display, GParamSpec *spec, LaminaxWindowTracker *tracker);

static void track_window (LaminaxWindowTracker *tracker, MetaWindow *window);
static void disassociate_window (LaminaxWindowTracker *tracker, MetaWindow *window);
LaminaxApp *meta_startup_sequence_get_app (MetaStartupSequence *sequence);

static void
Laminax_window_tracker_get_property (GObject    *gobject,
                            guint       prop_id,
                            GValue     *value,
                            GParamSpec *pspec)
{
  LaminaxWindowTracker *tracker = Laminax_WINDOW_TRACKER (gobject);
...
static void
Laminax_window_tracker_class_init (LaminaxWindowTrackerClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  gobject_class->get_property = Laminax_window_tracker_get_property;
  gobject_class->finalize = Laminax_window_tracker_finalize;
...
/**
 * Laminax_window_tracker_is_window_interesting:
 * @tracker: the LaminaxWindowTracker
 * @window: a #MetaWindow
 *
 * The LaminaxWindowTracker associates certain kinds of windows with
 ...
 */
gboolean
Laminax_window_tracker_is_window_interesting (LaminaxWindowTracker *tracker, MetaWindow *window)
{
    MetaWindowType type;
  ...
  switch (type)
    {
      ...
      case META_WINDOW_UTILITY:
        break;
      default:
        g_warning ("Laminax_window_tracker_is_window_interesting: default reached");
      break;
    }
 ...
 * return %NULL.
 *
 * Return value: (transfer full): A newly-referenced #LaminaxApp, or %NULL
 */
static LaminaxApp *
get_app_from_window_wmclass (MetaWindow  *window)
{
  LaminaxApp *app;
  LaminaxAppSystem *appsys;
  const char *wm_class;
  const char *wm_instance;

  appsys = Laminax_app_system_get_default ();
  ...
  */
  ...
  wm_instance = meta_window_get_wm_class_instance (window);
  app = Laminax_app_system_lookup_startup_wmclass (appsys, wm_instance);
  if (app != NULL)
    return g_object_ref (app);

  /* then try a match from WM_CLASS to StartupWMClass */
  wm_class = meta_window_get_wm_class (window);
  app = Laminax_app_system_lookup_startup_wmclass (appsys, wm_class);
  if (app != NULL)
    return g_object_ref (app);

  /* then try a match from WM_CLASS (instance part) to .desktop */
  app = Laminax_app_system_lookup_desktop_wmclass (appsys, wm_instance);
  if (app != NULL)
    return g_object_ref (app);
  ...
  app = Laminax_app_system_lookup_desktop_wmclass (appsys, wm_class);
  if (app != NULL)
    return g_object_ref (app);
 ...
 * @monitor: a #LaminaxWindowTracker
 * @window: a #MetaWindow
 *
 * Looks only at the given window, and attempts to determine
...
get_app_from_gapplication_id (MetaWindow  *window)
{
  ...
  app = Laminax_app_system_lookup_app (appsys, desktop_file);
  if (app)
    g_object_ref (app);
 ...
 */
static LaminaxApp*
get_app_from_window_group (LaminaxWindowTracker  *tracker,
                           MetaWindow          *window)
{
  LaminaxApp *result;
 ...
 * @tracker: a #LaminaxWindowTracker
 * @window: a #MetaWindow
 *
 ...
 */
static LaminaxApp *
get_app_from_window_pid (LaminaxWindowTracker  *tracker,
                         MetaWindow          *window)
{
  LaminaxApp *result;
  int pid;
  ...
  result = Laminax_window_tracker_get_app_from_pid (tracker, pid);
  if (result != NULL)
    g_object_ref (result);
  ...
  if (g_file_query_exists (file, NULL)) {
    ...
    if (result == NULL) {
      ...
      if (g_key_file_load_from_file (keyfile, info_filename, G_KEY_FILE_NONE, NULL)) {
        ...
        app = Laminax_app_system_lookup_flatpak_app_id (appsys, app_id);
 ...
 *
 * Returns: (transfer full): a #LaminaxApp, or NULL if none is found
 */
static LaminaxApp *
get_app_for_window (LaminaxWindowTracker    *tracker,
                    MetaWindow            *window)
{
  LaminaxApp *result = NULL;
  ...
  if (meta_window_is_remote (window))
    return _Laminax_app_new_for_window (window);
  ...
  if (startup_id)
    {
      ...
      sequences = Laminax_window_tracker_get_startup_sequences (tracker);
  ...
  if (result == NULL)
    result = _Laminax_app_new_for_window (window);

  return result;
}

static void
update_focus_app (LaminaxWindowTracker *self)
{
  MetaWindow *new_focus_win;
  LaminaxApp *new_focus_app;

  new_focus_win = meta_display_get_focus_window (self->global->meta_display);
  new_focus_app = new_focus_win ? Laminax_window_tracker_get_window_app (self, new_focus_win) : NULL;
...
static void
tracked_window_changed (LaminaxWindowTracker *self,
                        MetaWindow         *window)
{
...
on_wm_class_changed (MetaWindow  *window,
                     ...
                     gpointer     user_data)
{
  LaminaxWindowTracker *self = Laminax_WINDOW_TRACKER (user_data);
...
static void
track_window (LaminaxWindowTracker *self,
              MetaWindow      *window)
{
  LaminaxApp *app;

  if (!Laminax_window_tracker_is_window_interesting (self, window))
    return;
...
static void
Laminax_window_tracker_on_window_added (MetaWorkspace   *workspace,
                                   MetaWindow      *window,
                                   gpointer         user_data)
{
  LaminaxWindowTracker *self = Laminax_WINDOW_TRACKER (user_data);
...
static void
disassociate_window (LaminaxWindowTracker   *self,
                     MetaWindow        *window)
{
  LaminaxApp *app;
...
static void
Laminax_window_tracker_on_window_removed (MetaWorkspace   *workspace,
                                     MetaWindow      *window,
                                     gpointer         user_data)
{
  disassociate_window (Laminax_WINDOW_TRACKER (user_data), window);
}

void
load_initial_windows (LaminaxWindowTracker *tracker)
{
...
static void
Laminax_window_tracker_on_n_workspaces_changed (MetaWorkspaceManager *workspace_manager,
                                              GParamSpec           *pspec,
                                              gpointer              user_data)
{
  LaminaxWindowTracker *self = Laminax_WINDOW_TRACKER (user_data);
  ...
  for (l = workspaces; l; l = l->next)
    {
      ...
      g_signal_handlers_disconnect_by_func (workspace,
                                            Laminax_window_tracker_on_window_added,
                                            self);
      g_signal_handlers_disconnect_by_func (workspace,
                                            Laminax_window_tracker_on_window_removed,
                                            self);

      g_signal_connect (workspace, "window-added",
                        G_CALLBACK (Laminax_window_tracker_on_window_added), self);
      g_signal_connect (workspace, "window-removed",
                        G_CALLBACK (Laminax_window_tracker_on_window_removed), self);
    }
}

static void
init_window_tracking (LaminaxWindowTracker *self)
{
...
static void
Laminax_window_tracker_finalize (GObject *object)
{
  LaminaxWindowTracker *self = Laminax_WINDOW_TRACKER (object);

  g_hash_table_destroy (self->window_to_app);
  g_hash_table_destroy (self->launched_pid_to_app);

  G_OBJECT_CLASS (Laminax_window_tracker_parent_class)->finalize(object);
}

/**
 * Laminax_window_tracker_get_window_app:
 ...
 */
LaminaxApp *
Laminax_window_tracker_get_window_app (LaminaxWindowTracker *tracker,
                                     MetaWindow         *metawin)
{
...
/**
 * Laminax_window_tracker_get_app_from_pid:
 * @self; A #LaminaxAppSystem
 * @pid: A Unix process identifier
 *
 * Look up the application corresponding to a process.
 *
 * Returns: (transfer none): A #LaminaxApp, or %NULL if none
 */
LaminaxApp *
Laminax_window_tracker_get_app_from_pid (LaminaxWindowTracker *self,
                                       int                 pid)
{
  GSList *running = Laminax_app_system_get_running (Laminax_app_system_get_default());
  GSList *iter;
  LaminaxApp *result = NULL;

  for (iter = running; iter; iter = iter->next)
    {
      LaminaxApp *app = iter->data;
      GSList *pids = Laminax_app_get_pids (app);
      GSList *pids_iter;
...
void
_Laminax_window_tracker_add_child_process_app (LaminaxWindowTracker *tracker,
                                             GPid                pid,
                                             LaminaxApp           *app)
{
...
static void
set_focus_app (LaminaxWindowTracker  *tracker,
               LaminaxApp            *new_focus_app)
{
...
/**
 * Laminax_window_tracker_get_startup_sequences:
 * @tracker:
 *
 * Returns: (transfer none) (element-type MetaStartupSequence): Currently active startup sequences
 */
GSList *
Laminax_window_tracker_get_startup_sequences (LaminaxWindowTracker *self)
{
  MetaStartupNotification *sn = meta_display_get_startup_notification (self->global->meta_display);
...
meta_startup_sequence_get_app (MetaStartupSequence *sequence)
{
  ...
  appsys = Laminax_app_system_get_default ();
  app = Laminax_app_system_lookup_app (appsys, basename);
  g_free (basename);
  return app;
}
...
/**
 * Laminax_window_tracker_get_default:
 *
 * Return Value: (transfer none): The global #LaminaxWindowTracker instance
 */
LaminaxWindowTracker *
Laminax_window_tracker_get_default (void)
{
  static LaminaxWindowTracker *instance;

  if (instance == NULL)
    instance = g_object_new (Laminax_TYPE_WINDOW_TRACKER, NULL);
