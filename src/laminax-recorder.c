...
#include "Laminax-recorder-src.h"
...
struct _LaminaxRecorderClass
{
  GObjectClass parent_class;
};

struct _LaminaxRecorder {
  GObject parent;
...
static void recorder_set_stage    (LaminaxRecorder *recorder,
                                   ClutterStage  *stage);
static void recorder_set_framerate (LaminaxRecorder *recorder,
                                    int framerate);
static void recorder_set_pipeline (LaminaxRecorder *recorder,
                                   const char    *pipeline);
static void recorder_set_file_template (LaminaxRecorder *recorder,
                                        const char    *file_template);
static void recorder_set_draw_cursor (LaminaxRecorder *recorder,
                                      gboolean       draw_cursor);
...
static void recorder_remove_redraw_timeout (LaminaxRecorder *recorder);
...
G_DEFINE_TYPE(LaminaxRecorder, Laminax_recorder, G_TYPE_OBJECT);
...
static void
Laminax_recorder_init (LaminaxRecorder *recorder)
{
  /* Calling gst_init() is a no-op if GStreamer was previously initialized */
  gst_init (NULL, NULL);

  Laminax_recorder_src_register ();
...
static void
Laminax_recorder_finalize (GObject  *object)
{
  LaminaxRecorder *recorder = Laminax_RECORDER (object);
  ...
  G_OBJECT_CLASS (Laminax_recorder_parent_class)->finalize (object);
}
 ...
 */
static void
recorder_update_memory_used (LaminaxRecorder *recorder,
                             gboolean       repaint)
{
  guint memory_used = 0;
  GSList *l;
...
static void
recorder_add_redraw_timeout (LaminaxRecorder *recorder)
{
...
static void
recorder_remove_redraw_timeout (LaminaxRecorder *recorder)
{
  if (recorder->redraw_timeout != 0)
    {
...
static void
recorder_fetch_cursor_image (LaminaxRecorder *recorder)
{
  CoglTexture *texture;
  int width, height;
  int stride;
 ...
 */
static void
recorder_draw_cursor (LaminaxRecorder *recorder,
                      GstBuffer     *buffer)
{
  GstMapInfo info;
 ...
 */
static void
recorder_record_frame (LaminaxRecorder *recorder,
                       gboolean          paint)
{
  GstBuffer *buffer;
  ...
  * indicator from flashing between red and yellow. */
  ...
  if (n_captures == 1)
    ...
    image = Laminax_util_composite_capture_images (captures,
                                                 n_captures,
                                                 recorder->area.x,
                                                 recorder->area.y,
  ...
  Laminax_recorder_src_add_buffer (Laminax_RECORDER_SRC (recorder->current_pipeline->src), buffer);
...
static void
recorder_update_size (LaminaxRecorder *recorder)
{
...
static void
recorder_queue_redraw (LaminaxRecorder *recorder)
{
...
static void
recorder_update_pointer (LaminaxRecorder *recorder)
{
...
static void
recorder_add_update_pointer_timeout (LaminaxRecorder *recorder)
{
...
static void
recorder_remove_update_pointer_timeout (LaminaxRecorder *recorder)
{
  if (recorder->update_pointer_timeout)
    {
...
static void
recorder_connect_stage_callbacks (LaminaxRecorder *recorder)
{
...
static void
recorder_disconnect_stage_callbacks (LaminaxRecorder *recorder)
{
...
static void
recorder_set_stage (LaminaxRecorder *recorder,
                    ClutterStage  *stage)
{
...
static void
recorder_set_display (LaminaxRecorder *recorder,
                      MetaDisplay   *display)
{
...
static void
recorder_set_framerate (LaminaxRecorder *recorder,
                        int framerate)
{
...
static void
recorder_set_pipeline (LaminaxRecorder *recorder,
                       const char    *pipeline)
{
...
static void
recorder_set_file_template (LaminaxRecorder *recorder,
                            const char    *file_template)
{
...
static void
recorder_set_draw_cursor (LaminaxRecorder *recorder,
                          gboolean       draw_cursor)
{
...
static void
Laminax_recorder_set_property (GObject      *object,
                             guint         prop_id,
                             const GValue *value,
                             GParamSpec   *pspec)
{
...
static void
Laminax_recorder_get_property (GObject         *object,
                             guint            prop_id,
                             GValue          *value,
                             GParamSpec      *pspec)
{
...
static void
Laminax_recorder_class_init (LaminaxRecorderClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  gobject_class->finalize = Laminax_recorder_finalize;
  gobject_class->get_property = Laminax_recorder_get_property;
  gobject_class->set_property = Laminax_recorder_set_property;
...
{
  LaminaxRecorder *recorder = pipeline->recorder;
  GstCaps *caps;
 ...
 * LaminaxRecorderSrc element where we inject frames then additional elements
...
recorder_pipeline_add_source (RecorderPipeline *pipeline)
{
  ...
  if (sink_pad == NULL)
    {
      g_warning("LaminaxRecorder: pipeline has no unlinked sink pad");
      goto out;
    }

  pipeline->src = gst_element_factory_make ("Laminaxrecordersrc", NULL);
  ...
  if (!src_pad)
    {
      g_warning("LaminaxRecorder: can't get src pad to link into pipeline");
      goto out;
    }

  if (gst_pad_link (src_pad, sink_pad) != GST_PAD_LINK_OK)
    {
      g_warning("LaminaxRecorder: can't link to sink pad");
      goto out;
    }
 ...
 */
static int
recorder_open_outfile (LaminaxRecorder  *recorder,
                       char          **outfilename)
{
  const char *pattern;
  char *date_str, *path;
  ...
  if (date_str == NULL)
    {
      date_str = g_strdup_printf ("Laminax-%u", g_random_int ());
      g_warning ("Invalid filename template provided to LaminaxRecorder. Filename will be %s", date_str);
    }
...
recorder_pipeline_add_sink (RecorderPipeline *pipeline)
{
  ...
  if (!sink_pad)
    {
      g_warning("LaminaxRecorder: can't get sink pad to link pipeline output");
      goto out;
    }
 ...
 */
static void
recorder_pipeline_on_memory_used_changed (LaminaxRecorderSrc *src,
                                          GParamSpec       *spec,
...
static gboolean
recorder_open_pipeline (LaminaxRecorder *recorder)
{
  RecorderPipeline *pipeline;
  ...
  if (pipeline->pipeline == NULL)
    {
      g_warning ("LaminaxRecorder: failed to parse pipeline: %s", error->message);
      g_error_free (error);
      goto error;
    }
...
static void
recorder_close_pipeline (LaminaxRecorder *recorder)
{
...
/**
 * Laminax_recorder_new:
 * @stage: The #ClutterStage
 *
 * Create a new #LaminaxRecorder to record movies of a #ClutterStage
 *
 * Return value: The newly created #LaminaxRecorder object
 */
LaminaxRecorder     *
Laminax_recorder_new (ClutterStage  *stage)
{
  return g_object_new (Laminax_TYPE_RECORDER,
                       "stage",    stage,
                       NULL);
}

/**
 * Laminax_recorder_set_framerate:
 * @recorder: the #LaminaxRecorder
 ...
 */
void
Laminax_recorder_set_framerate (LaminaxRecorder *recorder,
                             int framerate)
{
  g_return_if_fail (Laminax_IS_RECORDER (recorder));

  recorder_set_framerate (recorder, framerate);
}

/**
 * Laminax_recorder_set_file_template:
 * @recorder: the #LaminaxRecorder
 ...
 * %%: A literal percent
 *
 * The default value is 'Laminax-%d%u-%c.ogg'.
 */
void
Laminax_recorder_set_file_template (LaminaxRecorder *recorder,
                                  const char    *file_template)
{
  g_return_if_fail (Laminax_IS_RECORDER (recorder));
...
void
Laminax_recorder_set_draw_cursor (LaminaxRecorder *recorder,
                                gboolean       draw_cursor)
{
  g_return_if_fail (Laminax_IS_RECORDER (recorder));
 ...
 * output file. (See Laminax_recorder_set_file_template().) However
 ...
 */
void
Laminax_recorder_set_pipeline (LaminaxRecorder *recorder,
                             const char    *pipeline)
{
  g_return_if_fail (Laminax_IS_RECORDER (recorder));
...
void
Laminax_recorder_set_area (LaminaxRecorder *recorder,
                            int               x,
                            int               y,
                            int               width,
                            int               height)
 ...
 */
gboolean
Laminax_recorder_record (LaminaxRecorder  *recorder,
                       char          **filename_used)
{
  g_return_val_if_fail (Laminax_IS_RECORDER (recorder), FALSE);
  ...
  meta_disable_unredirect_for_display (Laminax_global_get_display (Laminax_global_get ()));
...
/**
 * Laminax_recorder_close:
 * @recorder: the #LaminaxRecorder
 *
 * Stops recording. It's possible to call Laminax_recorder_record()
 ...
 */
void
Laminax_recorder_close (LaminaxRecorder *recorder)
{
  g_return_if_fail (Laminax_IS_RECORDER (recorder));
  ...
  meta_enable_unredirect_for_display (Laminax_global_get_display (Laminax_global_get ()));
 ...
 */
gboolean
Laminax_recorder_is_recording (LaminaxRecorder *recorder)
{
  g_return_val_if_fail (Laminax_IS_RECORDER (recorder), FALSE);
