...
#include <libLaminax-desktop/gnome-systemd.h>
...
static LaminaxGlobal *the_object = NULL;
...
G_DEFINE_TYPE(LaminaxGlobal, Laminax_global, G_TYPE_OBJECT);

static guint Laminax_global_signals [LAST_SIGNAL] = { 0 };

static void setup_log_handler (LaminaxGlobal *global);

static void
Laminax_global_set_property(GObject         *object,
                          guint            prop_id,
                          const GValue    *value,
                          GParamSpec      *pspec)
{
  LaminaxGlobal *global = Laminax_GLOBAL (object);

  switch (prop_id)
    {
    case PROP_STAGE_INPUT_MODE:
      Laminax_global_set_stage_input_mode (global, g_value_get_enum (value));
      break;
...
static void
Laminax_global_get_property(GObject         *object,
                          guint            prop_id,
                          GValue          *value,
                          GParamSpec      *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_OVERLAY_GROUP:
      ...
      break;
    case PROP_SCREEN:
      g_value_set_object (value, global->Laminax_screen);
      break;
    case PROP_DISPLAY:
...
static void
setup_notifications_service (LaminaxGlobal *global)
{
  gboolean disabled;

  /* notify-osd allows itself to be replaced as the notification handler. dunst does not,
     nor does Laminax. If we attempt to own and fail, Laminax is still 'on deck' to take
...
static void
Laminax_global_init (LaminaxGlobal *global)
{
  const char *datadir = g_getenv ("Laminax_DATADIR");
  const char *Laminax_js = g_getenv("Laminax_JS");
  char *imagedir, **search_path;

  if (!datadir)
    datadir = Laminax_DATADIR;
  ...
  global->userdatadir = g_build_filename (g_get_user_data_dir (), "Laminax", NULL);
  g_mkdir_with_parents (global->userdatadir, 0700);

  global->settings = g_settings_new ("org.Laminax");
  ...
  global->input_mode = Laminax_STAGE_INPUT_MODE_NORMAL;

  if (!Laminax_js)
    Laminax_js = JSDIR;
  search_path = g_strsplit (Laminax_js, ":", -1);
...
static void
Laminax_global_finalize (GObject *object)
{
  LaminaxGlobal *global = Laminax_GLOBAL (object);
  ...
  G_OBJECT_CLASS(Laminax_global_parent_class)->finalize (object);
}

static void
Laminax_global_class_init (LaminaxGlobalClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  gobject_class->get_property = Laminax_global_get_property;
  gobject_class->set_property = Laminax_global_set_property;
  gobject_class->finalize = Laminax_global_finalize;

  Laminax_global_signals[NOTIFY_ERROR] =
  ...
  Laminax_global_signals[SCALE_CHANGED] =
  ...
  Laminax_global_signals[SHUTDOWN] =
  ...
  g_object_class_install_property (gobject_class,
                                   ...
                                   g_param_spec_object ("display",
                                                        "Display",
                                                        "Metacity display object for Laminax",
                                                        META_TYPE_DISPLAY,
                                                        G_PARAM_READABLE));
  ...
  g_object_class_install_property (gobject_class,
                                   ...
                                   g_param_spec_object ("settings",
                                                        "Settings",
                                                        "GSettings instance for Laminax configuration",
                                                        G_TYPE_SETTINGS,
                                                        G_PARAM_READABLE));
  g_object_class_install_property (gobject_class,
                                   PROP_DATADIR,
                                   g_param_spec_string ("datadir",
                                                        "Data directory",
                                                        "Directory containing Laminax data files",
                                                        NULL,
                                                        G_PARAM_READABLE));
  g_object_class_install_property (gobject_class,
                                   PROP_IMAGEDIR,
                                   g_param_spec_string ("imagedir",
                                                        "Image directory",
                                                        "Directory containing Laminax image files",
                                                        NULL,
                                                        G_PARAM_READABLE));
  g_object_class_install_property (gobject_class,
                                   PROP_USERDATADIR,
                                   g_param_spec_string ("userdatadir",
                                                        "User data directory",
                                                        "Directory containing Laminax user data",
                                                        NULL,
                                                        G_PARAM_READABLE));
  g_object_class_install_property (gobject_class,
                                   PROP_FOCUS_MANAGER,
...
/**
 * _Laminax_global_init: (skip)
 * @first_property_name: the name of the first property
 * @...: the value of the first property, followed optionally by more
 *  name/value pairs, followed by %NULL
 *
 * Initializes Laminax global singleton with the construction-time
 * properties.
 *
 * There are currently no such properties, so @first_property_name should
 * always be %NULL.
 *
 * This call must be called before Laminax_global_get() and shouldn't be called
 * more than once.
 */
void
_Laminax_global_init (const char *first_property_name,
                    ...)
{
  ...
  the_object = Laminax_GLOBAL (g_object_new_valist (Laminax_TYPE_GLOBAL,
                                                  first_property_name,
                                                  argument_list));
...
focus_window_changed (MetaDisplay *display,
                      ...
                      gpointer     user_data)
{
  LaminaxGlobal *global = user_data;

  if (global->input_mode == Laminax_STAGE_INPUT_MODE_FOCUSED &&
      meta_display_get_focus_window (display) != NULL)
    Laminax_global_set_stage_input_mode (global, Laminax_STAGE_INPUT_MODE_NORMAL);
}

static void
Laminax_global_focus_stage (LaminaxGlobal *global)
{
  meta_focus_stage_window (global->meta_display, Laminax_global_get_current_time (global));
}

/**
 * Laminax_global_set_stage_input_mode:
 * @global: the #LaminaxGlobal
 * @mode: the stage input mode
 *
 * Sets the input mode of the stage; when @mode is
 * %Laminax_STAGE_INPUT_MODE_NONREACTIVE, then the stage does not absorb
 * any clicks, but just passes them through to underlying windows.
 * When it is %Laminax_STAGE_INPUT_MODE_NORMAL, then the stage accepts
 * clicks in the region defined by
 * Laminax_global_set_stage_input_region() but passes through clicks
 * outside that region. When it is %Laminax_STAGE_INPUT_MODE_FULLSCREEN,
 * the stage absorbs all input.
 *
 * When the input mode is %Laminax_STAGE_INPUT_MODE_FOCUSED, the pointer
 * is handled as with %Laminax_STAGE_INPUT_MODE_NORMAL, but additionally
 ...
 * will revert to %Laminax_STAGE_INPUT_MODE_NORMAL.
 *
 * Note that whenever a muffin-internal Gtk widget has a pointer grab,
 * Laminax behaves as though it was in
 * %Laminax_STAGE_INPUT_MODE_NONREACTIVE, to ensure that the widget gets
 * any clicks it is expecting.
 */
void
Laminax_global_set_stage_input_mode (LaminaxGlobal         *global,
                                      LaminaxStageInputMode  mode)
{
  g_return_if_fail (Laminax_IS_GLOBAL (global));
  ...
  if (mode == Laminax_STAGE_INPUT_MODE_NONREACTIVE)
    meta_x11_display_clear_stage_input_region (x11_display);
  else if (mode == Laminax_STAGE_INPUT_MODE_FULLSCREEN || !global->input_region)
  ...
  if (mode == Laminax_STAGE_INPUT_MODE_FOCUSED)
    Laminax_global_focus_stage (global);
...
/**
 * Laminax_global_set_cursor:
 * @global: A #LaminaxGlobal
 * @type: the type of the cursor
 *
 * Set the cursor on the stage window.
 */
void
Laminax_global_set_cursor (LaminaxGlobal *global,
                            LaminaxCursor type)
{
  MetaCursor ret_curs;
  ...
  switch (type)
    {
    case Laminax_CURSOR_DND_IN_DRAG:
      ret_curs = META_CURSOR_DND_IN_DRAG;
      break;
    case Laminax_CURSOR_DND_MOVE:
      ret_curs = META_CURSOR_DND_MOVE;
      break;
    case Laminax_CURSOR_DND_COPY:
      ret_curs = META_CURSOR_DND_COPY;
      break;
    case Laminax_CURSOR_DND_UNSUPPORTED_TARGET:
      ret_curs = META_CURSOR_DND_UNSUPPORTED_TARGET;
      break;
    case Laminax_CURSOR_POINTING_HAND:
      ret_curs = META_CURSOR_POINTING_HAND;
      break;
    case Laminax_CURSOR_RESIZE_BOTTOM:
      ret_curs = META_CURSOR_SOUTH_RESIZE;
      break;
    case Laminax_CURSOR_RESIZE_TOP:
      ret_curs = META_CURSOR_NORTH_RESIZE;
      break;
    case Laminax_CURSOR_RESIZE_LEFT:
      ret_curs = META_CURSOR_WEST_RESIZE;
      break;
    case Laminax_CURSOR_RESIZE_RIGHT:
      ret_curs = META_CURSOR_EAST_RESIZE;
      break;
    case Laminax_CURSOR_RESIZE_BOTTOM_RIGHT:
      ret_curs = META_CURSOR_SE_RESIZE;
      break;
    case Laminax_CURSOR_RESIZE_BOTTOM_LEFT:
      ret_curs = META_CURSOR_SW_RESIZE;
      break;
    case Laminax_CURSOR_RESIZE_TOP_RIGHT:
      ret_curs = META_CURSOR_NE_RESIZE;
      break;
    case Laminax_CURSOR_RESIZE_TOP_LEFT:
      ret_curs = META_CURSOR_NW_RESIZE;
      break;
    case Laminax_CURSOR_CROSSHAIR:
      ret_curs = META_CURSOR_CROSSHAIR;
      break;
    case Laminax_CURSOR_TEXT:
...
/**
 * Laminax_global_unset_cursor:
 * @global: A #LaminaxGlobal
 *
 * Unset the cursor on the stage window.
 */
void
Laminax_global_unset_cursor (LaminaxGlobal  *global)
{
  meta_display_set_cursor (global->meta_display, META_CURSOR_DEFAULT);
}

/**
 * Laminax_global_set_stage_input_region:
 * @global: the #LaminaxGlobal
 * @rectangles: (element-type Meta.Rectangle): a list of #MetaRectangle
 * describing the input region.
 *
 * Sets the area of the stage that is responsive to mouse clicks when
 * the stage mode is %Laminax_STAGE_INPUT_MODE_NORMAL (but does not change the
 * current stage mode).
 */
void
Laminax_global_set_stage_input_region (LaminaxGlobal *global,
                                     GSList      *rectangles)
{
  MetaRectangle *rect;
  XRectangle *rects;
   ...
   */
  Laminax_global_set_stage_input_mode (global, global->input_mode);
}

/**
 * Laminax_global_get_stage:
 *
 * Return value: (transfer none): The default #ClutterStage
 */
ClutterStage *
Laminax_global_get_stage (LaminaxGlobal  *global)
{
  return global->stage;
}

/**
 * Laminax_global_get_screen:
 *
 * Return value: (transfer none): The (Meta) LaminaxScreen
 */
LaminaxScreen *
Laminax_global_get_screen (LaminaxGlobal  *global)
{
  return global->Laminax_screen;
}

/**
 * Laminax_global_get_display:
 *
 * Return value: (transfer none): The default #MetaDisplay
 */
MetaDisplay *
Laminax_global_get_display (LaminaxGlobal  *global)
{
  return global->meta_display;
}

/**
 * Laminax_global_get_window_actors:
 *
 ...
 */
GList *
Laminax_global_get_window_actors (LaminaxGlobal *global)
{
  g_return_val_if_fail (Laminax_IS_GLOBAL (global), NULL);
...
global_stage_notify_width (GObject    *gobject,
                           ...
                           gpointer    data)
{
  LaminaxGlobal *global = Laminax_GLOBAL (data);
...
{
  Laminax_perf_log_event (Laminax_perf_log_get_default (),
...
static void
Laminax_fonts_init (ClutterStage *stage)
{
...
static void
update_scaling_factor (LaminaxGlobal  *global,
...
void
_Laminax_global_set_plugin (LaminaxGlobal *global,
                          MetaPlugin  *plugin)
{
  g_return_if_fail (Laminax_IS_GLOBAL (global));
  ...
  global->wm = Laminax_wm_new (plugin);
  ...
  global->Laminax_screen = Laminax_screen_new (global->meta_display);
  ...
  if (g_getenv ("Laminax_PERF_OUTPUT") != NULL)
    {
  ...
  Laminax_fonts_init (global->stage);
  ...
  // gdk_event_handler_set (gnome_Laminax_gdk_event_handler, global->stage, NULL);
...
/**
 * Laminax_global_dump_gjs_stack:
 * @global: A #LaminaxGlobal
 *
 * Prints out the gjs stack
 */
void
Laminax_global_dump_gjs_stack (LaminaxGlobal *global)
{
  gjs_dumpstack ();
}

GjsContext *
_Laminax_global_get_gjs_context (LaminaxGlobal *global)
{
...
static void
setup_log_handler (LaminaxGlobal *global)
{
...
static guint32
get_current_time_maybe_roundtrip (LaminaxGlobal *global)
{
  guint32 time;

  time = Laminax_global_get_current_time (global);
  if (time != CurrentTime)
    return time;
...
static ClutterActor *
get_key_focused_actor (LaminaxGlobal *global)
{
  ClutterActor *actor;
...
static void
sync_input_region (LaminaxGlobal *global)
{
...
/**
 * Laminax_global_begin_modal:
 * @global: a #LaminaxGlobal
 *
 * Grabs the keyboard and mouse to the stage window. The stage will
 * receive all keyboard and mouse events until Laminax_global_end_modal()
 ...
 *  window or alt-Tab window selection, or because Laminax_global_begin_modal()
 *  was previously called.
 */
gboolean
Laminax_global_begin_modal (LaminaxGlobal       *global,
                          guint32           timestamp,
...
/**
 * Laminax_global_end_modal:
 * @global: a #LaminaxGlobal
 *
 * Undoes the effect of Laminax_global_begin_modal().
 */
void
Laminax_global_end_modal (LaminaxGlobal *global,
                        guint32      timestamp)
{
  if (!meta_display_get_compositor (global->meta_display))
    return;
...
/**
 * Laminax_global_reexec_self:
 * @global: A #LaminaxGlobal
 *
 * Initiates the shutdown sequence.
 */
void
Laminax_global_reexec_self (LaminaxGlobal *global)
{
  meta_restart ("Restarting Laminax...");
}

/**
 * Laminax_global_real_restart:
 * @global: A #LaminaxGlobal
 *
 * Restart the current process.
 */
void 
Laminax_global_real_restart (LaminaxGlobal *global)
{
  GPtrArray *arr;
  gsize len;
  ...
  meta_display_close (global->meta_display,
                      Laminax_global_get_current_time (global));
 ...
 */
void
Laminax_global_notify_error (LaminaxGlobal  *global,
                           const char   *msg,
                           const char   *details)
{
 ...
 */
void
Laminax_global_get_pointer (LaminaxGlobal         *global,
                          int                 *x,
                          int                 *y,
 ...
 */
void
Laminax_global_set_pointer (LaminaxGlobal         *global,
                          int                 x,
                          int                 y)
{
  ClutterSeat *seat;
 ...
 */
void
Laminax_global_sync_pointer (LaminaxGlobal *global)
{
  int x, y;
  ClutterModifierType mods;
  ClutterEvent *event;
  ...
  Laminax_global_get_pointer (global, &x, &y, &mods);

  seat = clutter_backend_get_default_seat (clutter_get_default_backend ());
  event = clutter_event_new (CLUTTER_MOTION);

  event->motion.time = Laminax_global_get_current_time (global);
 ...
 */
GSettings *
Laminax_global_get_settings (LaminaxGlobal *global)
{
  return global->settings;
}

/**
 * Laminax_global_get_current_time:
 * @global: A #LaminaxGlobal
 *
 ...
 */
guint32
Laminax_global_get_current_time (LaminaxGlobal *global)
{
  guint32 time;
...
/**
 * Laminax_global_get_pid:
 *
 * Return value: the pid of the Laminax process.
 */
pid_t
Laminax_global_get_pid (LaminaxGlobal *global)
{
  return getpid();
}

/**
 * Laminax_global_get_md5_for_string:
 * @string: input string
 *
 * Return value: (transfer full): the MD5 sum for the given string
 */
gchar *
Laminax_global_get_md5_for_string (LaminaxGlobal *global, const gchar *string)
{
    return g_compute_checksum_for_string (G_CHECKSUM_MD5, string, -1);
}

static void
Laminax_global_app_launched_cb (GAppLaunchContext *context,
                                 GAppInfo          *info,
...
/**
 * Laminax_global_create_app_launch_context:
...
Laminax_global_create_app_launch_context (LaminaxGlobal *global)
{
  ...
  meta_launch_context_set_timestamp (context, Laminax_global_get_current_time (global));
  ...
  g_signal_connect (context,
                    "launched",
                    G_CALLBACK (Laminax_global_app_launched_cb),
                    NULL);
...
static void
schedule_leisure_functions (LaminaxGlobal *global)
{
...
/**
 * Laminax_global_begin_work:
 * @global: the #LaminaxGlobal
 *
 * Marks that we are currently doing work. This is used to to track
 * whether we are busy for the purposes of Laminax_global_run_at_leisure().
 * A count is kept and Laminax_global_end_work() must be called exactly
 * as many times as Laminax_global_begin_work().
 */
void
Laminax_global_begin_work (LaminaxGlobal *global)
{
  global->work_count++;
}

/**
 * Laminax_global_end_work:
 * @global: the #LaminaxGlobal
 *
 * Marks the end of work that we started with Laminax_global_begin_work().
 * If no other work is ongoing and functions have been added with
 * Laminax_global_run_at_leisure(), they will be run at the next
 * opportunity.
 */
void
Laminax_global_end_work (LaminaxGlobal *global)
{
...
/**
 * Laminax_global_run_at_leisure:
 * @global: the #LaminaxGlobal
 * @func: function to call at leisure
 * @user_data: data to pass to @func
 * @notify: function to call to free @user_data
 *
 * Schedules a function to be called the next time Laminax is idle.
 * Idle means here no animations, no redrawing, and no ongoing background
 * work. Since there is currently no way to hook into the Clutter master
 * clock and know when is running, the implementation here is somewhat
 * approximation. Animations done through Laminax's Tweener module will
 ...
 */
void
Laminax_global_run_at_leisure (LaminaxGlobal         *global,
                             LaminaxLeisureFunction func,
                             gpointer             user_data,
                             GDestroyNotify       notify)
{
...
/**
 * Laminax_global_segfault:
 * @global: the #LaminaxGlobal
 *
 * Crashes Laminax by causing a segfault
 */
void
Laminax_global_segfault (LaminaxGlobal *global)
{
  int *ptr = NULL;
 ...
 */
void
Laminax_global_alloc_leak (LaminaxGlobal *global, gint mb)
{
    gint i;
