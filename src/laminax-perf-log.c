...
typedef struct _LaminaxPerfEvent LaminaxPerfEvent;
typedef struct _LaminaxPerfStatistic LaminaxPerfStatistic;
typedef struct _LaminaxPerfStatisticsClosure LaminaxPerfStatisticsClosure;
typedef union  _LaminaxPerfStatisticValue LaminaxPerfStatisticValue;
typedef struct _LaminaxPerfBlock LaminaxPerfBlock;

/**
 * SECTION:Laminax-perf-log
 * @short_description: Event recorder for performance measurement
 *
 * LaminaxPerfLog provides a way for different parts of the code to
...
struct _LaminaxPerfLogClass
{
  GObjectClass parent_class;
};

struct _LaminaxPerfEvent
{
  guint16 id;
  char *name;
  char *description;
...
union _LaminaxPerfStatisticValue
{
  int i;
  gint64 x;
};

struct _LaminaxPerfStatistic
{
  LaminaxPerfEvent *event;

  LaminaxPerfStatisticValue current_value;
  LaminaxPerfStatisticValue last_value;

  guint initialized : 1;
  guint recorded : 1;
};

struct _LaminaxPerfStatisticsClosure
{
...
G_DEFINE_TYPE(LaminaxPerfLog, Laminax_perf_log, G_TYPE_OBJECT);
...
static void
Laminax_perf_log_init (LaminaxPerfLog *perf_log)
{
   ...
   * logging is enabled some time after starting Laminax */
  Laminax_perf_log_define_event (perf_log, "perf.setTime", "", "x");
   ...
   */
  Laminax_perf_log_define_event (perf_log, "perf.statisticsCollected",
...
static void
Laminax_perf_log_class_init (LaminaxPerfLogClass *class)
{
}

/**
 * Laminax_perf_log_get_default:
 *
 * Gets the global singleton performance log. This is initially disabled
 * and must be explicitly enabled with Laminax_perf_log_set_enabled().
 *
 * Return value: (transfer none): the global singleton performance log
 */
LaminaxPerfLog *
Laminax_perf_log_get_default (void)
{
  static LaminaxPerfLog *perf_log;

  if (perf_log == NULL)
    perf_log = g_object_new (Laminax_TYPE_PERF_LOG, NULL);
...
{
  LaminaxPerfLog *perf_log = data;

  Laminax_perf_log_collect_statistics (perf_log);

  return TRUE;
}

/**
 * Laminax_perf_log_set_enabled:
 * @perf_log: a #LaminaxPerfLog
 * @enabled: whether to record events
 *
 * Sets whether events are currently being recorded.
 */
void
Laminax_perf_log_set_enabled (LaminaxPerfLog *perf_log,
                            gboolean      enabled)
{
...
static LaminaxPerfEvent *
define_event (LaminaxPerfLog *perf_log,
              const char   *name,
              ...
              const char   *signature)
{
  ...
  event = g_slice_new (LaminaxPerfEvent);
 ...
 */
void
Laminax_perf_log_define_event (LaminaxPerfLog *perf_log,
                             const char   *name,
                             const char   *description,
                             const char   *signature)
{
...
static LaminaxPerfEvent *
lookup_event (LaminaxPerfLog *perf_log,
              const char   *name,
              const char   *signature)
{
  LaminaxPerfEvent *event = g_hash_table_lookup (perf_log->events_by_name, name);
...
static void
record_event (LaminaxPerfLog   *perf_log,
              gint64          event_time,
              LaminaxPerfEvent *event,
              const guchar   *bytes,
              size_t          bytes_len)
{
  LaminaxPerfBlock *block;
  size_t total_bytes;
  guint32 time_delta;
  guint32 pos;
  ...
  if (perf_log->blocks->tail == NULL ||
      total_bytes + ((LaminaxPerfBlock *)perf_log->blocks->tail->data)->bytes > BLOCK_SIZE)
    {
      block = g_new (LaminaxPerfBlock, 1);
      block->bytes = 0;
      g_queue_push_tail (perf_log->blocks, block);
    }
  else
    {
      block = (LaminaxPerfBlock *)perf_log->blocks->tail->data;
    }
 ...
 */
void
Laminax_perf_log_event (LaminaxPerfLog *perf_log,
                      const char   *name)
{
  LaminaxPerfEvent *event = lookup_event (perf_log, name, "");
  if (G_UNLIKELY (event == NULL))
    return;
 ...
 */
void
Laminax_perf_log_event_i (LaminaxPerfLog *perf_log,
                        const char   *name,
                        gint32        arg)
{
  LaminaxPerfEvent *event = lookup_event (perf_log, name, "i");
  if (G_UNLIKELY (event == NULL))
    return;
 ...
 */
void
Laminax_perf_log_event_x (LaminaxPerfLog *perf_log,
                        const char   *name,
                        gint64        arg)
{
  LaminaxPerfEvent *event = lookup_event (perf_log, name, "x");
  if (G_UNLIKELY (event == NULL))
    return;
 ...
 */
void
Laminax_perf_log_event_s (LaminaxPerfLog *perf_log,
                         const char   *name,
                         const char   *arg)
{
  LaminaxPerfEvent *event = lookup_event (perf_log, name, "s");
  if (G_UNLIKELY (event == NULL))
    return;
...
/**
 * Laminax_perf_log_define_statistic:
 * @perf_log: a #LaminaxPerfLog
 * @name: name of the statistic and of the corresponding event.
 *  This should follow the same guidelines as for Laminax_perf_log_define_event()
 ...
 * Laminax_perf_log_collect_statistics() is called explicitly.
 *
 ...
 * such as Laminax_perf_log_update_statistic_i(). This can be done
 * at any time, but would normally done inside a function registered
 * with Laminax_perf_log_add_statistics_callback(). These functions
 * are called immediately before statistics are recorded.
 */
void
Laminax_perf_log_define_statistic (LaminaxPerfLog *perf_log,
                                 const char   *name,
                                 const char   *description,
                                 const char   *signature)
{
  LaminaxPerfEvent *event;
  LaminaxPerfStatistic *statistic;
  ...
  statistic = g_slice_new (LaminaxPerfStatistic);
...
static LaminaxPerfStatistic *
lookup_statistic (LaminaxPerfLog *perf_log,
                  const char   *name,
                  const char   *signature)
{
  LaminaxPerfStatistic *statistic = g_hash_table_lookup (perf_log->statistics_by_name, name);
...
/**
 * Laminax_perf_log_update_statistic_i:
 * @perf_log: a #LaminaxPerfLog
 ...
 */
void
Laminax_perf_log_update_statistic_i (LaminaxPerfLog *perf_log,
                                   const char   *name,
                                   gint32        value)
{
  LaminaxPerfStatistic *statistic;
...
/**
 * Laminax_perf_log_update_statistic_x:
 * @perf_log: a #LaminaxPerfLog
 * @name: name of the statistic
 * @value: new value for the statistic
 *
 ...
 */
void
Laminax_perf_log_update_statistic_x (LaminaxPerfLog *perf_log,
                                   const char   *name,
                                   gint64        value)
{
  LaminaxPerfStatistic *statistic;
...
/**
 * Laminax_perf_log_add_statistics_callback:
 * @perf_log: a #LaminaxPerfLog
 ...
 * and call a function such as Laminax_perf_log_update_statistic_i()
 * to update the value that will be recorded.
 */
void
Laminax_perf_log_add_statistics_callback (LaminaxPerfLog               *perf_log,
                                        LaminaxPerfStatisticsCallback callback,
                                        gpointer                    user_data,
                                        GDestroyNotify              notify)
{
  LaminaxPerfStatisticsClosure *closure = g_slice_new (LaminaxPerfStatisticsClosure);
...
/**
 * Laminax_perf_log_collect_statistics:
 * @perf_log: a #LaminaxPerfLog
 *
 * Calls all the update functions added with
 * Laminax_perf_log_add_statistics_callback() and then records events
 * for all statistics, followed by a perf.statisticsCollected event.
 */
void
Laminax_perf_log_collect_statistics (LaminaxPerfLog *perf_log)
{
  ...
  for (i = 0; i < perf_log->statistics_closures->len; i++)
    {
      LaminaxPerfStatisticsClosure *closure;
  ...
  for (i = 0; i < perf_log->statistics->len; i++)
    {
      LaminaxPerfStatistic *statistic = g_ptr_array_index (perf_log->statistics, i);
      ...
      switch (statistic->event->signature[0])
        ...
        case 'x':
          ...
          break;
        default:
          g_warning("Laminax_perf_log_collect_statistics: default case");
          break;
        }
    }
 ...
 */
void
Laminax_perf_log_replay (LaminaxPerfLog            *perf_log,
                       ...
                       gpointer                 user_data)
{
  ...
  for (iter = perf_log->blocks->head; iter; iter = iter->next)
    {
      LaminaxPerfBlock *block = iter->data;
      guint32 pos = 0;
 ...
 */
gboolean
Laminax_perf_log_dump_events (LaminaxPerfLog   *perf_log,
                            GOutputStream  *out,
                            GError        **error)
{
  GString *output;
  ...
  for (i = 0; i < perf_log->events->len; i++)
    {
      LaminaxPerfEvent *event = g_ptr_array_index (perf_log->events, i);
 ...
 */
gboolean
Laminax_perf_log_dump_log (LaminaxPerfLog   *perf_log,
                         GOutputStream  *out,
                         GError        **error)
{
  ...
  Laminax_perf_log_replay (perf_log, replay_to_json, &closure);
