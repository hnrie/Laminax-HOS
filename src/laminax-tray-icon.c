...
struct _LaminaxTrayIconPrivate
{
  NaTrayChild *socket;
...
G_DEFINE_TYPE (LaminaxTrayIcon, Laminax_tray_icon, Laminax_TYPE_GTK_EMBED);

static void
Laminax_tray_icon_finalize (GObject *object)
{
  LaminaxTrayIcon *icon = Laminax_TRAY_ICON (object);

  g_free (icon->priv->title);
  g_free (icon->priv->wm_class);

  G_OBJECT_CLASS (Laminax_tray_icon_parent_class)->finalize (object);
}

static void
Laminax_tray_icon_constructed (GObject *object)
{
   ...
   * because Laminax may want to see their values from a
  ...
  if (icon_app_window == NULL)
    {
      g_warning ("Laminax tray: icon app window is gone");
      return;
    }
...
static void
Laminax_tray_icon_get_property (GObject         *object,
                              guint            prop_id,
                              GValue          *value,
                              GParamSpec      *pspec)
{
...
static void
Laminax_tray_icon_class_init (LaminaxTrayIconClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  g_type_class_add_private (klass, sizeof (LaminaxTrayIconPrivate));

  object_class->get_property = Laminax_tray_icon_get_property;
  object_class->constructed  = Laminax_tray_icon_constructed;
  object_class->finalize     = Laminax_tray_icon_finalize;
...
static void
Laminax_tray_icon_init (LaminaxTrayIcon *icon)
{
  icon->priv = G_TYPE_INSTANCE_GET_PRIVATE (icon, Laminax_TYPE_TRAY_ICON,
                                            LaminaxTrayIconPrivate);
}
 ...
 */
ClutterActor *
Laminax_tray_icon_new (LaminaxEmbeddedWindow *window)
{
  g_return_val_if_fail (Laminax_IS_EMBEDDED_WINDOW (window), NULL);

  return g_object_new (Laminax_TYPE_TRAY_ICON,
                       "window", window,
                       NULL);
}
...
/**
 * Laminax_tray_icon_handle_event:
 * @icon: a #LaminaxTrayIcon
 ...
 */
gboolean
Laminax_tray_icon_handle_event (LaminaxTrayIcon *icon,
                                 ...
                                 ClutterEvent     *event)
{
  ...
  if (remote_window == NULL)
    {
      g_warning ("Laminax tray: plug window is gone");
