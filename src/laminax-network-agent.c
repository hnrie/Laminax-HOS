...
#include "Laminax-network-agent.h"
...
struct _LaminaxNetworkAgentPrivate {
  /* <gchar *request_id, LaminaxAgentRequest *request> */
  GHashTable *requests;
};

G_DEFINE_TYPE_WITH_PRIVATE (LaminaxNetworkAgent, Laminax_network_agent, NM_TYPE_SECRET_AGENT_OLD)

static const SecretSchema network_agent_schema = {
    "org.freedesktop.NetworkManager.Connection",
    SECRET_SCHEMA_DONT_MATCH_NAME,
    {
        { Laminax_KEYRING_UUID_TAG, SECRET_SCHEMA_ATTRIBUTE_STRING },
        { Laminax_KEYRING_SN_TAG, SECRET_SCHEMA_ATTRIBUTE_STRING },
        { Laminax_KEYRING_SK_TAG, SECRET_SCHEMA_ATTRIBUTE_STRING },
        { NULL, 0 },
    }
};

static void
Laminax_agent_request_free (gpointer data)
{
  LaminaxAgentRequest *request = data;
...
static void
Laminax_agent_request_cancel (LaminaxAgentRequest *request)
{
  GError *error;
  LaminaxNetworkAgent *self;
...
static void
Laminax_network_agent_init (LaminaxNetworkAgent *agent)
{
  LaminaxNetworkAgentPrivate *priv;

  priv = agent->priv = Laminax_network_agent_get_instance_private (agent);
  priv->requests = g_hash_table_new_full (g_str_hash, g_str_equal,
                      g_free, Laminax_agent_request_free);
}

static void
Laminax_network_agent_finalize (GObject *object)
{
  LaminaxNetworkAgentPrivate *priv = Laminax_NETWORK_AGENT (object)->priv;
  GError *error;
  GHashTableIter iter;
  gpointer key;
  gpointer value;
  ...
  while (g_hash_table_iter_next (&iter, &key, &value))
    {
      LaminaxAgentRequest *request = value;
  ...
  G_OBJECT_CLASS (Laminax_network_agent_parent_class)->finalize (object);
}

static void
request_secrets_from_ui (LaminaxAgentRequest *request)
{
  ...
  for (l = items; l; l = g_list_next (l))
    {
      ...
      while (g_hash_table_iter_next (&iter, (gpointer *)&name, (gpointer *)&attribute))
        {
          if (g_strcmp0 (name, Laminax_KEYRING_SK_TAG) == 0)
            {
...
static void
Laminax_network_agent_get_secrets (NMSecretAgentOld                 *agent,
                                    NMConnection                     *connection,
                                    ...
                                    gpointer                          callback_data)
{
  LaminaxNetworkAgent *self = Laminax_NETWORK_AGENT (agent);
  LaminaxAgentRequest *request;
  GHashTable *attributes;
  char *request_id;
  ...
  if ((request = g_hash_table_lookup (self->priv->requests, request_id)) != NULL)
    {
       ...
       */
      Laminax_agent_request_cancel (request);
    }

  request = g_new0 (LaminaxAgentRequest, 1);
  ...
  attributes = secret_attributes_build (&network_agent_schema,
                                        Laminax_KEYRING_UUID_TAG, nm_connection_get_uuid (connection),
                                        Laminax_KEYRING_SN_TAG, setting_name,
                                        NULL);
...
void
Laminax_network_agent_add_vpn_secret (LaminaxNetworkAgent *self,
                                       gchar                *request_id,
                                       gchar                *setting_key,
                                       gchar                *setting_value)
{
  ...
  g_return_if_fail (Laminax_IS_NETWORK_AGENT (self));
...
void
Laminax_network_agent_set_password (LaminaxNetworkAgent *self,
                                     gchar                *request_id,
                                     gchar                *setting_key,
                                     gchar                *setting_value)
{
...
void
Laminax_network_agent_respond (LaminaxNetworkAgent         *self,
                                gchar                        *request_id,
                                LaminaxNetworkAgentResponse  response)
{
  ...
  if (response == Laminax_NETWORK_AGENT_USER_CANCELED)
    {
  ...
  if (response == Laminax_NETWORK_AGENT_INTERNAL_ERROR)
    {
      GError *error = g_error_new (NM_SECRET_AGENT_ERROR,
                                   NM_SECRET_AGENT_ERROR_FAILED,
  ...
  /* response == Laminax_NETWORK_AGENT_CONFIRMED */
...
void
Laminax_network_agent_search_vpn_plugin (LaminaxNetworkAgent   *self,
                                          const char          *service,
                                          ...
                                          gpointer             user_data)
{
  ...
  g_task_set_source_tag (task, Laminax_network_agent_search_vpn_plugin);
...
/**
 * Laminax_network_agent_search_vpn_plugin_finish:
 *
 * Returns: (nullable) (transfer full): The found plugin or %NULL
 */
NMVpnPluginInfo *
Laminax_network_agent_search_vpn_plugin_finish (LaminaxNetworkAgent  *self,
                                                 GAsyncResult       *result,
                                                 GError            **error)
{
  g_return_val_if_fail (Laminax_IS_NETWORK_AGENT (self), NULL);
...
static void
Laminax_network_agent_cancel_get_secrets (NMSecretAgentOld *agent,
                                           const gchar      *connection_path,
                                           const gchar      *setting_name)
{
  LaminaxNetworkAgent *self = Laminax_NETWORK_AGENT (agent);
  LaminaxNetworkAgentPrivate *priv = self->priv;
  gchar *request_id;
  LaminaxAgentRequest *request;
                              ...
                              gchar       **out_display_name)
{
  ...
  return secret_attributes_build (&network_agent_schema,
                                  Laminax_KEYRING_UUID_TAG, connection_uuid,
                                  Laminax_KEYRING_SN_TAG, setting_name,
                                  Laminax_KEYRING_SK_TAG, setting_key,
                                  NULL);
}
...
static void
Laminax_network_agent_save_secrets (NMSecretAgentOld                *agent,
                                     NMConnection                    *connection,
                                     const gchar                     *connection_path,
...
static void
Laminax_network_agent_delete_secrets (NMSecretAgentOld                  *agent,
                                       NMConnection                      *connection,
                                       const gchar                       *connection_path,
                                       ...
                                       gpointer                           callback_data)
{
  ...
  secret_password_clear (&network_agent_schema, NULL, delete_items_cb, r,
                         Laminax_KEYRING_UUID_TAG, uuid,
                         NULL);
}

void
Laminax_network_agent_class_init (LaminaxNetworkAgentClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
  NMSecretAgentOldClass *agent_class = NM_SECRET_AGENT_OLD_CLASS (klass);

  gobject_class->finalize = Laminax_network_agent_finalize;

  agent_class->get_secrets = Laminax_network_agent_get_secrets;
  agent_class->cancel_get_secrets = Laminax_network_agent_cancel_get_secrets;
  agent_class->save_secrets = Laminax_network_agent_save_secrets;
  agent_class->delete_secrets = Laminax_network_agent_delete_secrets;
