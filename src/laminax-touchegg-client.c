...
#include "Laminax-touchegg-client.h"
...
} LaminaxToucheggClientPrivate;

struct _LaminaxToucheggClient
{
    GObject parent_instance;
};

G_DEFINE_TYPE_WITH_PRIVATE (LaminaxToucheggClient, Laminax_touchegg_client, G_TYPE_OBJECT)
...
static void
emit_our_signal (LaminaxToucheggClient *client,
                 ...
                 GVariant               *params)
{
    ...
    g_debug ("LaminaxToucheggClient signal: %s: type %u, direction %u, progress %0.1f, fingers %d, device %u, elapsed_time %lu",
...
handle_signal (GDBusConnection *connection,
               ...
               gpointer         user_data)
{
    g_return_if_fail (Laminax_IS_TOUCHEGG_CLIENT (user_data));
    LaminaxToucheggClient *client = Laminax_TOUCHEGG_CLIENT (user_data);
    LaminaxToucheggClientPrivate *priv = Laminax_touchegg_client_get_instance_private (client);
...
static void init_connection (LaminaxToucheggClient *client);

static gboolean
retry_connection (gpointer data)
{
    LaminaxToucheggClient *client = Laminax_TOUCHEGG_CLIENT (data);
    LaminaxToucheggClientPrivate *priv = Laminax_touchegg_client_get_instance_private (client);

    g_debug ("LaminaxToucheggClient: retrying connection");
...
static void
connect_listener (LaminaxToucheggClient *client)
{
    LaminaxToucheggClientPrivate *priv = Laminax_touchegg_client_get_instance_private (client);
    g_debug ("LaminaxToucheggClient: connect_listener");
...
static void
init_connection (LaminaxToucheggClient *client)
{
    LaminaxToucheggClientPrivate *priv = Laminax_touchegg_client_get_instance_private (client);
    g_debug ("LaminaxToucheggClient: init_client");
...
static void
Laminax_touchegg_client_init (LaminaxToucheggClient *client)
{
    init_connection (client);
}

static void
Laminax_touchegg_client_dispose (GObject *object)
{
    LaminaxToucheggClient *client = Laminax_TOUCHEGG_CLIENT (object);
    LaminaxToucheggClientPrivate *priv = Laminax_touchegg_client_get_instance_private (client);
    g_debug ("LaminaxToucheggClient dispose (%p)", object);
    ...
    G_OBJECT_CLASS (Laminax_touchegg_client_parent_class)->dispose (object);
}

static void
Laminax_touchegg_client_finalize (GObject *object)
{
    g_debug ("LaminaxToucheggClient finalize (%p)", object);

    G_OBJECT_CLASS (Laminax_touchegg_client_parent_class)->finalize (object);
}

static void
Laminax_touchegg_client_class_init (LaminaxToucheggClientClass *klass)
{
    GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

    gobject_class->dispose = Laminax_touchegg_client_dispose;
    gobject_class->finalize = Laminax_touchegg_client_finalize;

    signals[GESTURE_BEGIN] =
          g_signal_new ("gesture-begin",
                        Laminax_TYPE_TOUCHEGG_CLIENT,
...
LaminaxToucheggClient *
Laminax_touchegg_client_new (void)
{
    LaminaxToucheggClient *client = g_object_new (Laminax_TYPE_TOUCHEGG_CLIENT, NULL);
