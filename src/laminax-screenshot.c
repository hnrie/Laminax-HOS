...
#include "Laminax-screenshot.h"

struct _LaminaxScreenshotClass
{
  GObjectClass parent_class;
};

struct _LaminaxScreenshot
{
...
typedef struct _screenshot_data {
  ...
  LaminaxScreenshotPickColorCallback pick_callback;
  ClutterColor pick_color;
} _screenshot_data;

G_DEFINE_TYPE(LaminaxScreenshot, Laminax_screenshot, G_TYPE_OBJECT);

static void
Laminax_screenshot_class_init (LaminaxScreenshotClass *screenshot_class)
{
  (void) screenshot_class;
}

static void
Laminax_screenshot_init (LaminaxScreenshot *screenshot)
{
  screenshot->global = Laminax_global_get ();
}
                   ...
                   cairo_rectangle_int_t  area)
{
  ...
  display = Laminax_global_get_display (Laminax_global_get ());
...
{
  MetaDisplay *display = Laminax_global_get_display (screenshot_data->screenshot->global);
                        ...
                        _screenshot_data *screenshot_data)
{
  ...
  result = g_simple_async_result_new (NULL, on_screenshot_written, (gpointer)screenshot_data, Laminax_screenshot_screenshot_window);
  g_simple_async_result_run_in_thread (result, write_screenshot_thread, G_PRIORITY_DEFAULT, NULL);
  g_object_unref (result);
}
...
/**
 * Laminax_screenshot_screenshot:
 * @screenshot: the #LaminaxScreenshot
 ...
 */
void
Laminax_screenshot_screenshot (LaminaxScreenshot *screenshot,
                             gboolean include_cursor,
                             const char *filename,
                             LaminaxScreenshotCallback callback)
{
  ...
  display = Laminax_global_get_display (screenshot->global);
  stage = CLUTTER_ACTOR (Laminax_global_get_stage (screenshot->global));
...
/**
 * Laminax_screenshot_screenshot_area:
 * @screenshot: the #LaminaxScreenshot
 ...
 */
void
Laminax_screenshot_screenshot_area (LaminaxScreenshot *screenshot,
                                  gboolean include_cursor,
                                  int x,
                                  int y,
                                  int width,
...
/**
 * Laminax_screenshot_screenshot_window:
 * @screenshot: the #LaminaxScreenshot
 ...
 */
void
Laminax_screenshot_screenshot_window (LaminaxScreenshot *screenshot,
                                    gboolean include_frame,
                                    gboolean include_cursor,
                                    const char *filename,
                                    LaminaxScreenshotCallback callback)
{
  MetaDisplay *display = Laminax_global_get_display (screenshot->global);
  MetaWindow *window = meta_display_get_focus_window (display);
  ClutterActor *stage;

  if (window == NULL || g_strcmp0 (meta_window_get_title (window), "Desktop") == 0)
  {
    Laminax_screenshot_screenshot (screenshot, include_cursor, filename, callback);
    return;
  }
 ...
 */
void
Laminax_screenshot_pick_color (LaminaxScreenshot *screenshot,
                                int x,
                                int y,
...
LaminaxScreenshot *
Laminax_screenshot_new (void)
{
  return g_object_new (Laminax_TYPE_SCREENSHOT, NULL);
}
