...
SCHEMAS = "org.Laminax.desklets.launcher"
LAUNCHER_KEY = "launcher-list"

HOME_DIR = os.path.expanduser("~")+"/"
CUSTOM_LAUNCHERS_PATH = os.path.join(GLib.get_user_data_dir(), "Laminax", "panel-launchers")
OLD_CUSTOM_LAUNCHERS_PATH = HOME_DIR + ".Laminax/panel-launchers/"
EDITOR_DIALOG_UI_PATH = "/usr/share/Laminax/desklets/launcher@Laminax.org/editorDialog.ui"
...
class EditorDialog:
    def __init__(self, desklet_id=-1):
        ...
        if desklet_id != - 1:
            ...
            self.name = launcher
            if self.name[:24] == "Laminax-custom-launcher":
                self.launcher_type = "Custom Application"

        self.tree = Gtk.Builder()
        self.tree.set_translation_domain('Laminax') # let it translate!
    ...
    def on_edit_ok_clicked(self, widget):
        ...
        if self.desklet_id == -1: # Add new launcher
            settings = Gio.Settings.new("org.Laminax")
            ...
            enabled_desklets.append("launcher@Laminax.org:%s:0:100" % self.desklet_id)
    ...
    def get_custom_path(self):
        ...
        i = 1
        while True:
            name = 'Laminax-custom-launcher-' + str(i) + '.desktop'
