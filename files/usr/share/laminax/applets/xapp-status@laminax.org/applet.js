...
class XAppStatusIcon {
    ...
    _onImageLoaded(cache, handle, actor, data=null) {
        if (handle !== this.icon_loader_handle) {
            global.logError(`xapp-status@Laminax.org: Icon or image seems out of sync (${this.name}`);
            return;
        }
    ...
    getEventPositionInfo(actor) {
        let allocation = Laminax.util_get_transformed_allocation(actor);
...
class LaminaxXAppStatusApplet extends Applet.Applet {
...
function main(metadata, orientation, panel_height, instance_id) {
    return new LaminaxXAppStatusApplet(orientation, panel_height, instance_id);
}
