...
const PRIVACY_SCHEMA = "org.Laminax.desktop.privacy";
        ...
        switch (this._action) {
            case "add_to_panel":
                if (!Main.AppletManager.get_role_provider_exists(Main.AppletManager.Roles.PANEL_LAUNCHER)) {
                    ...
                    enabled_applets.push("panel1:right:0:panel-launchers@Laminax.org:" + new_applet_id);
            ...
            case "remove_from_favorites":
                ...
                break;
            case "app_properties":
                Util.spawnCommandLine("Laminax-desktop-editor -mlauncher -o" + GLib.shell_quote(this._appButton.app.get_app_info().get_filename()));
                break;
            case "uninstall":
                Util.spawnCommandLine("/usr/bin/Laminax-remove-application '" + this._appButton.app.get_app_info().get_filename() + "'");
                break;
            case "offload_launch":
                try {
...
class LaminaxMenuApplet extends Applet.TextIconApplet {
    constructor(orientation, panel_height, instance_id) {
        ...
        edit_item.connect("activate", () => Util.spawnCommandLine("Laminax-menu-editor"));
        this._applet_context_menu.addMenuItem(edit_item);

        this.settings = new Settings.AppletSettings(this, "menu@Laminax.org", instance_id);
        ...
        this._canUninstallApps = GLib.file_test("/usr/bin/Laminax-remove-application", GLib.FileTest.EXISTS);
        ...
        // The reason we do is in case the Laminax icon theme is the same as the one specified in GTK itself (in .config)
    ...
    _launch_editor() {
        Util.spawnCommandLine("Laminax-menu-editor");
    }
    ...
    _getNewVectorInfo() {
        ...
        if (LaminaxMenuApplet.DEBUG_VMASK) {
    ...
    _keepMaskActive() {
        ...
        if (Math.abs(Math.hypot(x_dist, y_dist)) < LaminaxMenuApplet.MIN_MOVEMENT) {
            return false;
        }
    ...
    _enableVectorMask(actor) {
        ...
        this.vector_update_loop = Mainloop.timeout_add(LaminaxMenuApplet.POLL_INTERVAL, this._maskPollTimeout.bind(this));
    }
    ...
    _refreshApps() {
        ...
        this._allAppsCategoryButton = new CategoryButton(this, null, _("All Applications"), "Laminax-all-applications", true);
    ...
    _refreshSystemButtons() {
        ...
        button.activate = () => {
            ...
            let screensaver_settings = new Gio.Settings({ schema_id: "org.Laminax.desktop.screensaver" });
            let screensaver_dialog = Gio.file_new_for_path("/usr/bin/Laminax-screensaver-command");
            if (screensaver_dialog.query_exists(null)) {
                if (screensaver_settings.get_boolean("ask-for-away-message")) {
                    Util.spawnCommandLine("Laminax-screensaver-lock-dialog");
                }
                else {
                    Util.spawnCommandLine("Laminax-screensaver-command --lock");
                }
            }
            else {
    ...
    _buildSidebar() {
        ...
        this.userIcon.connect('button-press-event', () => {
            this.menu.toggle();
            Util.spawnCommandLine("Laminax-settings user");
        });
    ...
    _display() {
        ...
        this.a11y_settings = new Gio.Settings({ schema_id: "org.Laminax.desktop.a11y.applications" });
        this.a11y_settings.connect("changed::screen-magnifier-enabled", this._updateVFade.bind(this));
        this.a11y_mag_settings = new Gio.Settings({ schema_id: "org.Laminax.desktop.a11y.magnifier" });
...
function main(metadata, orientation, panel_height, instance_id) {
    return new LaminaxMenuApplet(orientation, panel_height, instance_id);
}
