...
MELANGE_DBUS_NAME = "org.Laminax.Melange"
MELANGE_DBUS_PATH = "/org/Laminax/Melange"
...
class MelangeApp(Gtk.Application):
    def __init__(self):
        Gtk.Application.__init__(self,
                                 application_id="org.Laminax.Melange",
    ...
    def construct_window(self):
        ...
        headerbar.set_subtitle("Laminax Troubleshooting Tool")
        ...
        self.status_label.set_markup(" <span foreground='red'>[ Laminax is OFFLINE! ]</span> ")
        self.status_label.set_tooltip_text("The connection to Laminax is broken")
        self.status_label.set_no_show_all(True)
        table.attach(self.status_label, column, column+1, 1, 2, 0, 0, 1)
        column += 1

        box = Gtk.HBox()
        settings = Gio.Settings(schema_id="org.Laminax.desktop.keybindings")
    ...
    def create_action_button(self):
        restart_func = lambda junk: os.system("nohup Laminax --replace > /dev/null 2>&1 &")
        ...
        menu.append(Gtk.SeparatorMenuItem())
        menu.append(self.create_menu_item('Restart Laminax', restart_func))
        menu.append(self.create_menu_item('Crash Laminax', crash_func))
        menu.append(self.create_menu_item('Reset Laminax Settings', self.on_reset_clicked))
        menu.append(Gtk.SeparatorMenuItem())
    ...
    def on_reset_clicked(self, menu_item):
        dialog = Gtk.MessageDialog(self.window, 0,
                                   ...
                                   "Reset all Laminax settings to default?")
        dialog.set_title("Warning: Trying to reset all Laminax settings!")

        response = dialog.run()
        dialog.destroy()
        if response == Gtk.ResponseType.YES:
            os.system("gsettings reset-recursively org.Laminax &")
...
if __name__ == "__main__":
    setproctitle("Laminax-looking-glass")
    app = MelangeApp()
    app.run(sys.argv)
    exit(0)
