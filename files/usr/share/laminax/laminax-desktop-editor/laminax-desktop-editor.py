...
sys.path.insert(0, '/usr/share/Laminax/Laminax-menu-editor')
from cme import util

sys.path.insert(0, '/usr/share/Laminax/Laminax-settings/bin')
...
PANEL_LAUNCHER_PATH = os.path.join(GLib.get_user_data_dir(), "Laminax", "panel-launchers")
OLD_PANEL_LAUNCHER_PATH = os.path.join(home, ".Laminax", "panel-launchers")

EXTENSIONS = (".png", ".xpm", ".svg")

DEFAULT_ICON_NAME = "Laminax-panel-launcher"
...
class ItemEditor(object):
    ...
    def __init__(self, item_path=None, callback=None, destdir=None, icon_size=24):
        ...
        self.builder.set_translation_domain('Laminax') # let it translate!
...
class LauncherEditor(ItemEditor):
    ui_file = '/usr/share/Laminax/Laminax-desktop-editor/launcher-editor.ui'
...
class DirectoryEditor(ItemEditor):
    ui_file = '/usr/share/Laminax/Laminax-desktop-editor/directory-editor.ui'
...
class LaminaxLauncherEditor(ItemEditor):
    ui_file = '/usr/share/Laminax/Laminax-desktop-editor/launcher-editor.ui'
    ...
    def check_custom_path(self):
        ...
        if self.item_path is None or "Laminax-custom-launcher" not in self.item_path:
            i = 1
            while True:
                name = os.path.join(PANEL_LAUNCHER_PATH, 'Laminax-custom-launcher-' + str(i) + '.desktop')
                old_name = os.path.join(OLD_PANEL_LAUNCHER_PATH, 'Laminax-custom-launcher-' + str(i) + '.desktop')
    ...
    def load(self):
        super(LaminaxLauncherEditor, self).load()
        self.set_text('name-entry', "Name")
        self.set_text('exec-entry', "Exec")
        self.set_text('comment-entry', "Comment")
        self.set_check('terminal-check', "Terminal")
...
class Main:
    def __init__(self):
        ...
        if options.mode == "Laminax-launcher" and len(args) < 1:
            parser.error("Laminax-launcher mode must have the following syntax:\n"
                         "Laminax-desktop-editor -mLaminax-launcher [-ffoo.desktop] <json-path>")

        self.tree = CMenu.Tree.new("Laminax-applications.menu", CMenu.TreeFlags.INCLUDE_NODISPLAY)
        ...
        if options.mode == "Laminax-launcher":
        ...
        if self.mode == "directory":
            ...
            editor.dialog.show_all()
        elif self.mode == "Laminax-launcher":
            editor = LaminaxLauncherEditor(self.orig_file, self.panel_launcher_cb, icon_size=self.icon_size)
            editor.dialog.show_all()
...
if __name__ == "__main__":
    setproctitle("Laminax-desktop-editor")
