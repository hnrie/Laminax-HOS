...
settings_dir = os.path.join(GLib.get_user_config_dir(), 'Laminax', 'spices')
old_settings_dir = f'{home}/.Laminax/configs/'

URL_SPICES_HOME = "https://Laminax-spices.linuxmint.com"
URL_MAP = {
...
class Spice_Harvester(GObject.Object):
    ...
    def __init__(self, collection_type, window=None):
        ...
        self.cache_folder = os.path.join(GLib.get_user_cache_dir(), 'Laminax', 'spices', self.collection_type)

        if self.themes:
            self.settings = Gio.Settings.new('org.Laminax.theme')
        ...
        if self.themes:
            ...
            self.install_folder = f'{home}/.local/share/Laminax/{self.collection_type}s/'
            self.spices_directories = (f'/usr/share/Laminax/{self.collection_type}s/', self.install_folder)
        ...
        try:
            if not self.actions:
                dbus_path = 'org.Laminax'
                gsetting = '/org/Laminax'
            else:
                dbus_path = 'org.Nemo'
                gsetting = '/org/Nemo'
    ...
    def _on_proxy_ready(self, obj, result, data=None):
        try:
            ...
            if self._proxy.get_name_owner():
                ...
                print("org.Laminax proxy created, but no owner - is Laminax running?")
        except GLib.Error as e:
            print(f"Could not establish proxy for org.Laminax: {e}")
